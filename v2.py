...
home_dir = Path.home()
config_path = f'{home_dir}/.local/share/smithy-pda'
...
def webui():
    ...
    <body>
        <div>
            <input id="user_input" type="text" placeholder="Type here">
            <button onclick="sendMessage()">Send</button>
        </div>
        <div>
            <p>Smithy: <span id="smithy_message">{{ smithy }}</span></p>
        </div>
        {% if request_flag %}
        <div>
            <button onclick="doConfirm()">Confirm</button>
            <button onclick="doDeny()">Deny</button>
        </div>
        {% endif %}
        <script>
            function sendMessage(){
                fetch('/send', {
                    ...
                    body: JSON.stringify({input: document.getElementById('user_input').value})
                }).then(()=>window.location.reload());
            }
            function doConfirm(){
                fetch('/confirm', {method: 'POST'}).then(()=>window.location.reload());
            }
            function doDeny(){
                fetch('/deny', {method: 'POST'}).then(()=>window.location.reload());
            }
        </script>
    </body>
    </html>
    """
    return render_template_string(html, smithy=smithy_message, request_flag=request)
